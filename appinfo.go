...
func (a *App) appInfo() any {
	return appInfoDTO{
		...
		Capabilities: []string{"secrets", "categories", "generate", "export", "audit", "updates", "ui.state", "ui.press", "ui.key", "ui.input"},
		Errors: []errorInfo{
		...
		AXTree: axNode{
			...
			Children: []axNode{
				...
				},
				{
					...
					Name:        "Unlock",
					ID:          "unlock",
					Description: "Shown while the vault is locked. A start hub (create / open / recent vaults) leads to the password screen; back-to-start returns to the hub. Native file dialogs are NOT reachable via /v1/ui — use POST /v1/unlock instead.",
					Children: []axNode{
						{Role: "button", Name: "Create a new vault", Testid: "goto-create", Action: "open the create-vault form", Risk: riskNavigation},
						{Role: "button", Name: "Open an existing vault", Testid: "browse-vault", Action: "pick a vault file (native dialog), then show its password screen", Risk: riskSafe},
						{Role: "list", Name: "Recent vaults", Testid: "recent-vault-<n>", Description: "up to 3 recently opened vaults (recent-vault-0..2); pressing one opens its password screen", Action: "choose that vault", Risk: riskSafe},
						{Role: "text", Name: "Vault path", Testid: "vault-path", Description: "the chosen vault file (password screen)"},
						...
						{Role: "button", Name: "Back", Testid: "back-to-start", Action: "return to the start hub", Risk: riskNavigation},
				...
				},
				{
					...
					Children: []axNode{
						...
						{Role: "combobox", Name: "Category filter", Testid: "category-filter", Action: "show only secrets of one category (empty value = all)", Risk: riskSafe},
						{Role: "button", Name: "Add Password", Testid: "add-secret", Action: "open the full-window editor with an empty secret", Risk: riskSafe},
						{Role: "button", Name: "Categories", Testid: "open-categories", Action: "open the Categories view", Risk: riskNavigation},
						{Role: "list", Name: "Secrets", Testid: "secret-list", Description: "one row per secret; rows have testid secret-<id> and per-row copy buttons"},
						{Role: "button", Name: "Secret row", Testid: "secret-<id>", Action: "open that secret in the editor", Risk: riskSafe},
						{Role: "button", Name: "Copy login", Testid: "copy-user-<id>", Action: "copy that secret's username to the clipboard", Risk: riskSafe},
						{Role: "button", Name: "Copy password (row)", Testid: "copy-pass-<id>", Action: "copy that secret's password to the clipboard (value never shown)", Risk: riskSensitive},
						{Role: "button", Name: "Open URL", Testid: "open-url-<id>", Action: "open that secret's URL in the default browser; present only when the secret has a URL", Risk: riskExternal},
						...
						{Role: "button", Name: "Cancel", Testid: "cancel-editor", Action: "discard the editor's changes and return to the list", Risk: riskSafe},
				...
				},
				{
					...
					Children: []axNode{
						...
						{Role: "textbox", Name: "Toast duration", Testid: "toast-seconds", Action: "seconds a toast stays on screen (0 disables toasts)", Risk: riskSafe},
						...
						{Role: "button", Name: "Change master password", Testid: "nav-master-password", Action: "open the change-master-password view", Risk: riskNavigation},
						...
						{Role: "switch", Name: "Automatic update checks", Testid: "update-autocheck", Action: "toggle checking GitHub once a day on launch (off by default; checking calls the network)", Risk: riskSafe},
						{Role: "button", Name: "Check for updates", Testid: "update-check", Action: "ask GitHub Releases for a newer version now; result also at GET /v1/update", Risk: riskExternal},
						{Role: "status", Name: "Update status", Testid: "update-status", Description: "outcome of the last update check"},
						{Role: "text", Name: "Release notes", Testid: "update-notes", Description: "present only while an update is available"},
						{Role: "button", Name: "Install and restart", Testid: "update-install", Action: "download, verify, replace the app and RESTART it; present only while an update is available", Risk: riskDestructive},
						{Role: "button", Name: "Skip this version", Testid: "update-skip", Action: "silence this version; present only while an update is available", Risk: riskSafe},
						{Role: "button", Name: "Remind me later", Testid: "update-later", Action: "snooze the notice for 7 days; present only while an update is available", Risk: riskSafe},
				...
				},
				{
					...
					Name:        "Categories",
					ID:          "categories",
					OpenedBy:    "open-categories",
					Description: "Manage categories as a DRAFT: name edits (cat-name-<id> inputs), color swatches (cat-color-<id>-<0..7>), Delete (cat-delete-<id>) and Add only take effect when save-categories is pressed; cancel-categories or back discards everything.",
					Children: []axNode{
						{Role: "button", Name: "Back", Testid: "back", Action: "return to the vault", Risk: riskNavigation},
						{Role: "textbox", Name: "New category", Testid: "new-category", Risk: riskSafe},
						{Role: "button", Name: "Add", Testid: "add-category", Action: "append the typed category to the draft (not saved yet)", Risk: riskSafe},
						{Role: "button", Name: "Save", Testid: "save-categories", Action: "persist all draft changes (adds, renames, colors, deletes)", Risk: riskSensitive},
						{Role: "button", Name: "Cancel", Testid: "cancel-categories", Action: "discard the draft and return to the vault", Risk: riskSafe},
					},
				},
				{
					Role:        "view",
					...
					ID:          "masterpassword",
					OpenedBy:    "nav-master-password",
					Description: "Opened from Settings → Advanced.",
					Children: []axNode{
